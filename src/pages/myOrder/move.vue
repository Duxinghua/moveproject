<template>
  ...
  >
    ...
    <div
      class='demand'
      v-if='orderDetail.orderLargeList && orderDetail.orderLargeList.length>0'
    >
      <p class='way'><span>大件物品</span><span>平台标准计价</span></p>
      <div class='ul'>
        <div
          class='li'
          v-for="(item,index,) in orderDetail.orderLargeList"
          :key="index"
        ><span>{{item.catType}}</span><span>{{item.catItem}}*{{item.qty}}</span></div>
      </div>
      <!-- <p class='way'><span>搬运楼层</span></p> -->
      <div class='ul'>
        <div
          class='li'
          v-for="(val,key,i) in orderDetail.orderPriceMap"
          :key="i"
        ><span>{{val.propName}}</span><span>{{val.remarks}}</span></div>
      </div>
    </div>
    <div class='remark'>
      <p>订单备注</p>
      ...
      >
        <img
          v-for='(item,index) in orderDetail.orderPicList'
          :key="index"
          :src='item.picUrl'
          style='width:25%;margin-right:2%;margin-top:20px;height:140px'
          @click='prediv(index,orderDetail.orderPicList)'
        ></img>
      </div>
    </div>
    <div class="name">
    ...
    <div class='contact'>
      <span>联系电话</span><span>{{orderDetail.receiverMobileNo}}
        <i

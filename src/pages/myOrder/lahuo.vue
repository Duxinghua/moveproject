<template>
  <div class='lahuo'>
    ...
    <div
      class='demand'
      v-if="Object.keys(orderDetail.orderPriceMap).length>0 || orderDetail.orderOtherList.length>0"
    >
      <div
        class="other"
        v-if="Object.keys(orderDetail.orderPriceMap).length>0"
      >
        <p class='way'><span>计价方式</span><span>{{orderDetail.priceType == 'STANDARD' ? '平台标准计价' : '与司机协商计价'}}</span></p>
        <div class='ul'>
          <div
            class='li'
            v-for="(val,key,i) in orderDetail.orderPriceMap"
            :key="i"
          ><span>{{key}}</span><span>{{val.catItem}}</span></div>
          <div
            class='li'
            v-if="orderDetail.largeGoodsCnt>0"
          ><span>大件物品数量</span><span>{{orderDetail.largeGoodsCnt}}</span></div>
        </div>
      </div>

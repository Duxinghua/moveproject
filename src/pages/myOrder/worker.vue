<template>
  <div class='worked'>
    ...
    <h4>{{orderDetail.orderStatus | orderStatus_worker}}</h4>
    <div
      class='calling'
    ...
    >
      <div v-if="orderDetail.custDriverInfoSimpleVo">
        ...
        <div
          class='rates'
          v-if="orderDetail.serverType=='HIRE_WORKER'"
        >
          <div>
            <span>{{orderDetail.custDriverInfoSimpleVo.name || orderDetail.custDriverInfoSimpleVo.nickName}}</span>
            <div class="rate">
              <div
        ...
        <div
          ...
          v-else
        >
          <p v-if='orderDetail.custDriverInfoSimpleVo'><span >{{formatName(orderDetail.custDriverInfoSimpleVo.plateNumber) || ''}}</span><span>{{orderDetail.carType | carType}}</span></p>
          <div>
    ...
    <div class="address">
      <div class="left_wrap">
        <div class="start">
          <!-- <img src='../../assets/img/dingwei.png' class='dingwei'></img> -->
          <!-- 	<div class='people'>
						<span>{{orderDetail.receiverName }}</span><span>{{orderDetail.receiverMobileNo }}</span>
					</div> -->
          <p class="addressName">{{orderDetail.workerRegion}}</p>
          <p class="detailAddress">{{orderDetail.workerAddress}}</p>
        </div>
      </div>
    </div>
    <div class='demand'>
      <div class='ul'>
        <div class='li'><span>工种</span><span>{{orderDetail.workTypeName }}-{{orderDetail.workTypeName2}}</span></div>
        <!-- <div class='li'><span>详细需求</span><span>{{orderDetail.orderDescribe}}</span></div> -->
        <div class='li'><span>需要人数</span><span>{{orderDetail.workerUserCnt }}人</span></div>
        <div class='li'><span>工期</span><span>{{orderDetail.workerTimeQty }}天</span></div>
      </div>
    </div>
    <div class='remark'>
      <p>订单备注</p>
...
TopNav
  ...
  methods: {
    ...
    formatName(name) {
      ...
      return  name.substr(0, 2) + "***" + name.substr(5, name.split("").length);
    },
  },
};
</script>

...
export default {
  ...
  methods: {
    likeGo () {
      ...
      this.$router.push({name: 'MyHhr', query: {is_partner: this.userInfo.is_partner, is_apply: this.userInfo.is_apply}})
      // } else {
    ...
    likeClickHandler (arg) {
      ...
      if (arg === 'xf') {
        ...
        // if (this.userInfo.is_apply == 1) {
        this.$router.push({name: Links, query: {is_partner: this.userInfo.is_partner, is_apply: this.userInfo.is_apply}})
        // } else {
        // this.$toast('您是合伙人下线，不能申请合伙人')
        // }
      } else {
